...
python3 template.py "your question" "option1" "option2" "option3" "option4"python3 template.py "your question" "option1" "option2" "option3" "option4"
ALL THE BEST!!
"""
...
def llm_function(model,tokenizer,q,a,b,c,d):
    ...
    '''
    # Initialize variables for tracking best answer
    options = [a, b, c, d]
    max_score = float('-inf')
    final_output = 'A'  # Default to A if all else fails
    
    # Check each option individually using logit scores
    for idx, option in enumerate(options):
        prompt = f"Question: {q}\nIs '{option}' the correct answer?\nRespond with YES or NO."
        ...
        with torch.no_grad():
            ...
            # Compare YES/NO scores
            yes_score = logits[yes_token_id].item()
            no_score = logits[no_token_id].item()
            
            # Consider both the YES score and the difference between YES and NO scores
            confidence_score = yes_score - no_score
            
            if confidence_score > max_score:
                max_score = confidence_score
                final_outpu = chr(65 + idx)
